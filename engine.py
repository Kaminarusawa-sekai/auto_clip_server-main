...
class Engine:
    ...
    def clip_video(self, tag, video_path, start_time, end_time, new_video_name, keep_full_audio, audio_volume):
        ...
        try:
            ...
            if video_clip.audio==None:
                ...
                if keep_full_audio:
                    half_volumn_effect=afx.MultiplyVolume(audio_volume)
                    video_clip.audio.with_effects([half_volumn_effect])
                    video_clip.audio.write_audiofile(video_path+"-audio.mp3")
                    ...
                    # audio_clip_path = None
                    # video_clip = video_clip.set_audio(video_clip_audio)
            video_clip = video_clip.subclipped(start_time=start_time,end_time=end_time)
            #新视频片段文件夹
    ...
    def merge_videos(self, video_clips_map, clips_keys, output_path, video_title, bgm_obj, audio_obj, subtitle_obj, tail_obj):
        ...
        #读取封面图片
        cover_image = ImageClip(output_path+".png",duration=0.1)
        # cover_image.duration=0.1
        # cover_image = cover_image.set_position(("center","center"))
        # cover_image.pos=("center","center")
        cover_image = cover_image.with_position(("center","center"))
        #将封面和第一个片段合并
        watermarked_clip = CompositeVideoClip([first_clip, cover_image])
        # 先输出避免错误
        watermarked_clip_path=first_clip_path+"marked.mp4"
        watermarked_clip.write_videofile(watermarked_clip_path, codec='libx264')
        watermarked_video_clip=VideoFileClip(watermarked_clip_path)
        #添加到待合成数组中
        video_clips.append(watermarked_video_clip)
        ...
        for key in clips_keys[1:]: 
            ...
            newfadein=vfx.FadeIn(duration=0.3)
            newfadeout=vfx.FadeOut(duration=0.3)
            new_clip = video_clip.with_effects([newfadein,newfadeout])
        ...
        if bgm_obj:
            ...
            aloop=afx.AudioLoop(duration=final_time)
            bgm_audio_track = bgm_clip.with_effects([aloop])
        ...
        if audio_obj:
            ...
            half_volumn_effect=afx.MultiplyVolume(audio_file_vol)
            main_track = AudioFileClip(audio_file).with_effects([half_volumn_effect])
        ...
        if tail_obj:
            ...
            # tail_audio_track = tail_clip.audio.volumex(tail_obj["音量"])
                tail_clip.audio=tail_audio_track
    ...
    def add_subtitle(self, subtitle_file, final_clip, font_name, font_size, font_color):
        ...
        generator = lambda txt: TextClip(font=font_name, text=txt,font_size=font_size, color=font_color,interline=-1, size=(1000,500),vertical_align="bottom", method='caption')
        # generator = lambda txt: TextClip(txt, font=font_name, fontsize=font_size, color=font_color, interline=-1, size=(1000,500),vertical_align="bottom", method='caption')
        # generator = lambda txt: TextClip(txt, font=font_name, fontsize=font_size, color=font_color)

        subs = SubtitlesClip(subtitles=subtitle_file, make_textclip=generator,encoding='utf-8')
        # subs = subs.set_duration(final_clip.duration)
        subs.duration=final_clip.duration
        print(f'subs.duration: {subs.duration}')
        # final_video = CompositeVideoClip([final_clip, subs.with_position(('center','center'))],use_bgclip=True)
        final_video = CompositeVideoClip([final_clip, subs.with_position(('center','center'))])
        return final_video
...
def random_file_in_subfolder(folder_path, prefix):
    ...
    if not selected_file:
        print(f"No files found in the selected subfolder '{folder_path}'.")
        return
...
def random_number(min, max):
    ...
    min = round(min, 2)
    max = round(max, 2)
    return random.randint(int(min*100), int(max*100)) / 100
